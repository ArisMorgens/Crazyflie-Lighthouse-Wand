...
class SwarmFlasher:
    ...
    def __init__(self, config_file: str, firmware_base: Optional[str] = None):
        ...
        self.firmware_base = Path(firmware_base) if firmware_base else Path(self.config.get('firmware_base', './crazyflie-firmware'))
...
Examples:
  ...
  %(prog)s --range 1-6 --app-type pilot
    ...
    parser.add_argument(
        '--platform',
        choices=['cf2', 'cf21bl', 'bolt'],
        help='Filter by platform type'
    )
    parser.add_argument(
        '--app-type',
        choices=['wand', 'receiver', 'receiver_hlc'],
